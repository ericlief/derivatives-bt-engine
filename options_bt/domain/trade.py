...
class TradeResult:
    ...
    Designed to work efficiently with pandas DataFrames.
    """
    trade_id: int
    quantity: int
    ...
    @classmethod
    def from_dataframe_row(cls, row: pd.Series) -> 'TradeResult':
        """Create TradeResult from a DataFrame row."""
        return cls(**row.to_dict())

    @staticmethod
    def to_dataframe(results: list['TradeResult']) -> pd.DataFrame:
