...
class Backtester:
    ...
    def run(
        ...
        valid_signals = signals[signals['margin_required'] <= config.max_margin_utilization]
    ...
    def _execute_backtest(self,
        ...
        trade_counter = 0
        option_bp = trade_manager.option_bp
        open_positions = []
        trade_results = []
        skipped_trades = 0
        ...
        for trade_signal in trade_signals.itertuples():
            ...
            if len(open_positions) >= self.trade_manager.max_positions:
                skipped_trades += 1
                continue

            # Create new trade from signal
            trade = trade_manager.build_trade_from_signal(trade_signal, config)
