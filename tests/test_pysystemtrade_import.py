...
def test_build_sidecar_imports_typed_data_manifest_and_qa(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert con.execute(
            "SELECT count(*) FROM daily.adjusted_prices"
        ).fetchone()[0] == 2
        assert con.execute(
        ...
        selections = dict(
            (stream, (selected, eod, fallback))
            for stream, selected, eod, fallback in con.execute(
                "SELECT * FROM qa.daily_selection"
            ).fetchall()
        )
        assert selections == {
            "adjusted_prices": (2, 2, 0),
            "carry": (1, 1, 0),
            "marks": (2, 2, 0),
        }
    finally:
...
def test_daily_tables_choose_2300_or_last_complete_snapshot(tmp_path: Path) -> None:
    source = _write_source_tree(tmp_path)
    with (source / "multiple_prices_csv" / "TEST.csv").open(
        "a", encoding="utf-8"
    ) as handle:
        handle.write(
            "2020-01-03 20:00:00,100,20200200,102,20200100,103,20200200\n"
            "2020-01-03 23:00:00,,20200200,103,20200100,104,20200200\n"
            "2020-01-04 20:00:00,102,20200200,104,20200100,105,20200200\n"
        )
    with (source / "adjusted_prices_csv" / "TEST.csv").open(
        "a", encoding="utf-8"
    ) as handle:
        handle.write(
            "2020-01-03 20:00:00,1\n"
            "2020-01-03 23:00:00,2\n"
            "2020-01-04 20:00:00,3\n"
        )
    output = tmp_path / "pysystemtrade_reference.duckdb"
    build_sidecar(source, output)
    ...
    try:
        mark = con.execute(
            "SELECT source_timestamp, selection_policy FROM daily.marks "
            "WHERE trade_date = DATE '2020-01-03'"
        ).fetchone()
        carry = con.execute(
            "SELECT source_timestamp, selection_policy FROM daily.carry "
            "WHERE trade_date = DATE '2020-01-03'"
        ).fetchone()
        fallback = con.execute(
            "SELECT source_timestamp, selection_policy FROM daily.marks "
            "WHERE trade_date = DATE '2020-01-04'"
        ).fetchone()
    finally:
        con.close()

    assert mark == (datetime(2020, 1, 3, 23), "eod_2300")
    assert carry == (datetime(2020, 1, 3, 20), "last_complete")
    assert fallback == (datetime(2020, 1, 4, 20), "last_complete")
