...
def mock_data():
    ...
    end_date = date(2023, 1, 31)  # Shortened to just January since all our tests use January dates
    ...
    for i, d in enumerate(dates):  # step through  dates
        for strike in strikes:
            current_price = underlying_prices[i]
            days_to_expiry = (end_date - d).days
    ...
    option_chain_data = pl.DataFrame(option_chain_data)
    data = {
            'option_chain': option_chain_data,
            'underlying_price_history': pl.DataFrame({'date': dates, 'close': underlying_prices})
            }
    return data
...
def mock_backtester(mocker):
    return mocker.patch('derivatives_bt_engine.domain.backtester.Backtester', autospec=True)

@pytest.fixture(scope='module')
def mock_option_signal_generator(mocker):
    return mocker.patch('derivatives_bt_engine.domain.option_signal_generator.OptionSignalGenerator', autospec=True)
