...
def clean_signal_rows(rows: Iterable[Mapping], run_id: str, *, as_of=None) -> list[dict]:
    ...
    for source in rows:
        ...
        row = {
            ...
            'g_fast': _number(_value(source, 'g_fast')),
            'g_slow': _number(_value(source, 'g_slow')),
            'a_co': _number(_value(source, 'a_co')),
            'a_re': _number(_value(source, 'a_re')),
            'g_blend': _number(_value(source, 'g_blend')),
            'g_sig': _number(_value(source, 'signal', 'g_sig')),
            'ts_fast': _number(_value(source, 'ts_fast')),
            'ts_slow': _number(_value(source, 'ts_slow')),
            'ts': _number(_value(source, 'ts')),
            'contin_sig': _number(_value(source, 'contin_signal', 'contin_sig')),
            'ts_reg': _value(source, 'ts_regime', 'ts_reg'),
            'day_std': _number(_value(source, 'daily_std', 'day_std')),
            'hv': hv,
            'dd_pct': _number(_value(source, 'dd_pct')),
            'risk_sc': _number(_value(source, 'risk_scalar', 'risk_sc')),
            'reg_disc': _number(_value(source, 'regime_discount', 'reg_discount', 'reg_disc')),
            'vx_sc': _number(_value(source, 'vix_scalar', 'vx_sc')),
            'comb_sc': _number(_value(source, 'combined_scalar', 'comb_sc', 'scalar')),
            'cur_con': _number(_value(source, 'current_contracts', 'prior_contracts', 'cur_con')),
            'frac_con': fractional_contracts,
            'tgt_con': target_contracts,
            'max_con': _number(_value(source, 'max_contracts', 'max_con')),
            'rnd_gap': rounding_gap,
